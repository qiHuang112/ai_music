...
- `just_audio_platform_interface` 的 MethodChannel 消息不会把 `AudioSource.tag` 里的 `MediaItem` 传给 HarmonyOS 原生插件；AI Music 通过 `com.qi.ai_music.ohos_media_controls` 私有通道只在 OpenHarmony 平台同步当前 `MediaItem` 的 title/artist/artUri/duration，用于系统播控中心展示。
- 曲目切换和 metadata 读取完成后，同步 title/artist/duration/mediaImage；封面优先使用 Dart 当前曲目的 `artUri`，其次用 `AVMetadataExtractor.fetchAlbumCover()` 抽取当前音频文件内嵌封面，最后才降级到默认图。
- 播放模式和收藏状态也通过同一个私有通道闭环：应用内变化时同步 `loopMode/isFavorite` 到 AVSession；系统播控触发 `setLoopMode/setTargetLoopMode/toggleFavorite` 时回到 Dart 的 `MusicController`，复用现有播放模式和收藏持久化逻辑。
